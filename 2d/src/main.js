...
const reset_game = (new_game = true) => {
  ...
  game_state.enemy_speed_multiplier = new_game
    ? ENEMY_NEW_GAME_SPEED_MULTIPLIER
    : ENEMY_NEW_GAME_SPEED_MULTIPLIER +
      game_state.round * ENEMY_SPEED_INCREASE_CONTINUOUS;
  game_state.ring_speed_modifier = new_game
    ? RING_STARTING_SPEED_MULTIPLIER
    : RING_STARTING_SPEED_MULTIPLIER +
      game_state.round * RING_SPEED_INCREASE_PER_ROUND;
  ...
  setTimeout(
    () => spawn_enemies(game_state.max_enemies, 1.0),
    ENEMY_INITIAL_SPAWN_DELAY_MS,
  );
};
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.code === "Backspace") {
    game_state.autopilot_on = !game_state.autopilot_on;
...
const update_game = (current_time) => {
  ...
  if (game_state.game_over) {
    if (game_state.entice_animation_t < 0) {
      ...
      game_state.entice_animation_t = Math.min(
        1,
        game_state.entice_animation_t + dt / ENTICE_ANIM_DURATION,
      );
  ...
  const active_keys = game_state.entice_mode ? brain_keys : keys_pressed;
